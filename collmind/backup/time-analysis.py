...
for embeddings_month, current_date in tqdm(gen_sent_embeddings(collection_name, start_date, end_date), total=12 * time_period.years + time_period.months):
    ...
    batch = pd.read_feather(join('transform', collection_name.lower(), model_name, f"batch-{current_date.strftime('%m%y')}.arrow"))
...
# %%
results_dir = join("ctfidf", collection_name.lower())
